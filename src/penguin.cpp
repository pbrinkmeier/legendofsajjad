...
namespace los {
	Penguin::Penguin(SDL_Renderer *renderer, const signed short posx, const signed short posy)
		...
		SDL_Surface *deathCropped = SurfaceEditor::crop(sf, rand() % 2 * m_width, 3 * m_height, m_width, m_height);
		SDL_Surface *emptySurface = SurfaceCreator::createEmptySurface(m_width, m_height);
	...
	void Penguin::update(const double tslf, float xOffset, float yOffset, signed short windowW, signed short windowH) {
		...
		m_posx += m_posxDir * tslf * 80.0f + xOffset;
		m_posy += m_posyDir * tslf * 80.0f + yOffset;
